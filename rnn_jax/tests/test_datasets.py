...
def test_to_forecasting(dataset_func):
    ...
    x, y = datasets.to_forecasting(x, forecast=5)  # type: ignore
...
def test_to_forecasting_with_test(dataset_func):
    ...
    x, xt, y, yt = datasets.to_forecasting(x, forecast=5, test_size=10)  # type: ignore
...
def test_from_aeon_classification():
    ...
    assert X_rpy.shape == (n_timeseries, n_timesteps, n_dimensions)  # type: ignore
    assert X_rpy[0, 2, 1] == np.pi  # type: ignore
    ...
    with pytest.raises(TypeError):
        datasets.from_aeon_classification(X_aeon_invalid)  # type: ignore

    X_aeon_array_like = range(10)
    with pytest.raises(ValueError):
        datasets.from_aeon_classification(X_aeon_array_like)  # type: ignore
