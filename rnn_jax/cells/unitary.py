...
class UnitaryEvolutionRNNCell(BaseCell):
    ...
    def __init__(
        ...
        *,
        ...
        self.v1 = jr.uniform(v1_key1, (hdim,), minval=-1, maxval=1) + 1j * jr.uniform(
            v1_key2, (hdim,), minval=-1, maxval=1
        )
        v2_key1, v2_key2 = jr.split(subkeys[1])
        self.v2 = jr.uniform(v2_key1, (hdim,), minval=-1, maxval=1) + 1j * jr.uniform(
            v2_key2, (hdim,), minval=-1, maxval=1
        )
        ...
        if permutation_type == "random":
            ...
            raise ValueError(
                f"allowed values for permutation_type are {allowed_permutations}, got {permutation_type}"
            )
        self.perm = perm
