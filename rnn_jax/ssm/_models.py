...
class DeepStateSpaceModelEncoder(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        assert len(mixers) == len(layers), (
            "number of layers and mixers must be the same"
        )
        ...
        if in_projection:
            ...
            self.in_projection = eqx.nn.Linear(
                in_features=in_dim, out_features=projected_dim, key=in_key
            )
        ...
        if out_dim is not None:
            self.out_projection = eqx.nn.Linear(
                in_features=self.model_dim[-1], out_features=out_dim, key=out_key
            )
        else:
