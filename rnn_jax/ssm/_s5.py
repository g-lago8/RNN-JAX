...
def leg_s_matrix(N):
    ...
    for n, k in product(range(N), range(N)):
        if n > k:
            A[n, k] = (2.0 * float(n) + 1.0) ** (1 / 2) * (2.0 * k + 1.0) ** (1 / 2)
        elif n == k:
            A[n, k] = n + 1
        # if n < k keep it 0
    return -A
    ...
    def _init_Lambda(self, n_blocks, init_type):
        ...
        if init_type == "leg-n":
            ...
            raise NotImplementedError(
                "The only possible initialization is 'leg-n' for now."
            )

    def discretize(self, seq_len):
        Lambda_bar = jnp.exp(self.Lambda * self.Delta)
        zoh_in_scaling = 1 / self.Lambda * (Lambda_bar - 1)
        W_in_bar = einops.einsum(
            self.W_in, zoh_in_scaling, "state_dim in_dim, state_dim -> state_dim in_dim"
        )
